...
DATASETS = ['control', 'lesion', 'pseudohealthy']
MODALITIES = {'control':MODALITIES_CONTROL, 'lesion':MODALITIES_LESION, 'pseudohealthy':MODALITIES_CONTROL}

batch_size = {'control':1, 'lesion':2, 'pseudohealthy':1}
...
def train(paths_dict, model, transformation, device, save_path, opt):
    ...
    while continue_training:
        ...
        #for phase in ['validation','training']:
            ...
            running_loss_pseudohealthy = 0.0
            ...
            for _ in tqdm(ind_batch[phase]):
                ...
                # PseudoHealthy data (T1) 
                batch_pseudohealthy_T1 = next(dataloaders['pseudohealthy'][phase])
                labels_pseudohealthy_T1 = batch_pseudohealthy_T1['label'][DATA].to(device).type(torch.cuda.IntTensor)
                inputs_pseudohealthy_T1 =  {'T1':batch_pseudohealthy_T1['T1'][DATA].to(device)}
                ...
                bs_pseudohealthy = inputs_pseudohealthy_T1['T1'].shape[0]
                ...
                with torch.set_grad_enabled(phase == 'training'):
                    # Concatenating the 3 predictions made on the T1 for a faster computation
                    input_T1 = {'T1':torch.cat([inputs_control['T1'],inputs_lesion_T1['T1'],inputs_pseudohealthy_T1['T1']],0)}
                    outputs_T1, _ = model(input_T1)

                    # Deconcatenating the 3 predictions
                    outputs_control = outputs_T1[:bs_control,...]
                    outputs_lesion_T1 = outputs_T1[bs_control:bs_control+bs_lesion,...]
                    output_pseudohealthy_T1 = outputs_T1[-bs_pseudohealthy:,...]
                    ...
                    loss_pseudohealthy = jaccard_tissue(output_pseudohealthy_T1, labels_pseudohealthy_T1)
                    loss_intermod = jaccard_tissue(outputs_lesion, outputs_lesion_T1, is_prob=True)


                    if epoch>opt.warmup and phase=='training':
                        loss = 1/2*(loss_control + loss_pseudohealthy) + loss_lesion + loss_intermod  
                    else:
                        loss = 1/2*(loss_control + loss_pseudohealthy) + loss_lesion                      
                ...
                running_loss_pseudohealthy = loss_pseudohealthy.item()
            ...
            epoch_loss_pseudohealthy = running_loss_pseudohealthy / epoch_samples
            ...
            print('{}  Loss Seg PseudoHealthy: {:.4f}'.format(
                phase, epoch_loss_pseudohealthy))
...
def main():
    ...
    split_path['pseudohealthy'] = opt.split_pseudohealthy
    for dataset in DATASETS:
    ...
    path_file['pseudohealthy'] = opt.path_pseudohealthy

    splits = ['training', 'validation']
    paths_dict = dict()

    for dataset in DATASETS:
        add_name = '_sym' if dataset=='pseudohealthy' else ''
        ...
        for split in splits:
            for subject in list_file[split]:
                ...
                for modality in MODALITIES[dataset]:
                    subject_data.append(Image(modality, path_file[dataset]+subject+modality+add_name+'.nii.gz', torchio.INTENSITY))
                if split in ['training', 'validation']:
                    subject_data.append(Image('label', path_file[dataset]+subject+'Label'+add_name+'.nii.gz', torchio.LABEL))
...
def parsing_data():
    ...
    parser.add_argument('--split_pseudohealthy',
                        type=str,
                        default='./splits/BRATS_sym/5_samples.csv')
    ...
    parser.add_argument('--path_pseudohealthy',
                        type=str,
                        default='../data/BRATS2018_sym/')
